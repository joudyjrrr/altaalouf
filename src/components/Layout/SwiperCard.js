        ...
        <div className="w-full flex flex-col ps-5">
          <div className="flex justify-between overflow-visible max-h-[50px]">
            <h3
              className={`text-[32px] font-bold leading-10  ${
                cardData.title !== "Zero Standard" && "mt-4"
              }`}
            >
              {cardData.title}
            </h3>
          ...
          <div className="mt-8">
            {cardData.features.map((item, index) => (
              <p
                key={index}
                className="text-[15.3px] font-medium mt-[.4rem] flex items-center "
              >
                <Image
                  src={arrowIcon}
                  alt="icon"
          ...
          <Button className="button-border z-10 hover:bg-main_button  p-[21px_18px] max-w-[235px] m-auto mt-4">

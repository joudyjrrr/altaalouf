  ...
  return (
    <>
      <div
        className={` 
        w-full pt-24 
        max-sLg:hidden mx-auto 
        justify-center items-center grid 
        grid-cols-1 sm:grid-cols-2
        md:grid-cols-5  md:px-8 gap-4 
        place-content-center ${className} `}
      >
    ...
    >
      ...
      {showButton && (
        <Link href={`/our-team/${image.name}`}>
          ...
          right-[1px] w-[60px] h-[50px] 
          rounded-br-[1.15rem] rounded-tl-[1.3rem]
      ...
      left-[0px] h-[45px] font-bold text-lg
       text-white"
      >
        {image.name}
      </div>
